...
**Status**: CRITICAL (Phase 5 User Testing Discovery)

**Discovery**: 2025-11-13 User testing in playground/
- Command: `ptrk init test-project --with-ci`
- Expected: .github/workflows/build.yml created with CI workflow template
- Actual: .github/workflows/ directory created (empty), no YML files generated
- Impact: Users cannot set up CI/CD pipeline without manual workflow creation

**Root Cause**:
- `lib/picotorokko/commands/init.rb`: `--with-ci` flag handler incomplete
- SPEC.md defines workflow content, but template generation logic missing

**Fix Scope**:
- Create workflow template files (or inline generation logic)
- Integrate into Init#execute when --with-ci flag present
- Test all option combinations (--with-ci alone, --with-ci + --with-mrbgem, etc.)

**Priority**: HIGH — Blocks user CI/CD setup
...
- Expected: Internal call to `ptrk mrbgems generate MyGem`, mrbgems/ directory created
- Actual: Flag ignored, no mrbgem generation, mrbgems/ directory not created
- Impact: Users cannot auto-generate mrbgem templates during project initialization

**Root Cause**:
- `lib/picotorokko/commands/init.rb`: `--with-mrbgem` option not parsed or handled
- No internal delegation to `ptrk mrbgems generate`

**Fix Scope**:
- Parse `--with-mrbgem` option (support multiple gems: --with-mrbgem Gem1 --with-mrbgem Gem2)
- Invoke `ptrk mrbgems generate GEM_NAME` for each specified gem (after project structure created)
- Handle errors and validate mrbgem names
- Update tests for all option combinations

**Priority**: HIGH — Blocks user mrbgem initialization

**Note**: `ptrk mrbgems generate TestGem` works standalone (verified in user testing), so focus is on integration into Init command
...
- ✅ Test ptrk init in playground/ (project with all options)
  ...
  - **Result**: Basic directory structure generated correctly ✓
- ✅ Verify all generated files are correct and functional
  - ✅ storage/home/app.rb (sample application)
  - ✅ Gemfile (dependencies)
  - ✅ README.md (detailed 157-line user guide)
  - ✅ .picoruby-env.yml (environment config)
  - ✅ .gitignore (correct exclusions)

#### ❌ Discovered Issues (Infrastructure Blockers)

- ❌ [TODO-INFRASTRUCTURE-CI-WORKFLOWS] CI workflow YML not generated (see Known Issues)
  - Expected: .github/workflows/build.yml
  - Actual: .github/workflows/ directory created empty
  - **Impact**: Users cannot run GitHub Actions CI/CD

- ❌ [TODO-INFRASTRUCTURE-MRBGEM-GENERATION] --with-mrbgem flag not processed (see Known Issues)
  - Expected: mrbgems/ directory with MyGem structure
  - Actual: Flag ignored, no mrbgem generated
  - **Impact**: Users cannot auto-generate mrbgems during init

#### ⏸️ Deferred Tests (Blocked by Infrastructure Issues)

- [ ] Test GitHub Actions workflow CI/CD execution — *Blocked by [TODO-INFRASTRUCTURE-CI-WORKFLOWS]*
- [ ] Verify mrbgem generation and compilation — *Blocked by [TODO-INFRASTRUCTURE-MRBGEM-GENERATION]*

#### 📝 Phase 5 Next Steps

1. **Priority 1**: Resolve [TODO-INFRASTRUCTURE-CI-WORKFLOWS] and [TODO-INFRASTRUCTURE-MRBGEM-GENERATION]
2. **Priority 2**: Re-test ptrk init with fixed implementations
...
- ❌ CI workflow auto-generation works (blocked, to be fixed Phase 5)
- ❌ mrbgem auto-generation works (blocked, to be fixed Phase 5)
